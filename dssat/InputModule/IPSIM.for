...
!  05/04/2020 ZZ Add option to read KCB from file in !SIM CONTROLS
C-----------------------------------------------------------------------
...
C
     ...
     &        ISWPHO,ISWPOT,ISWDIS,ISWCHE,ISWTIL, ICO2, IKCB
         IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEX,LINEXP)
...
!            ISWCHE = 'N'
         ENDIF
		 IF (IKCB .EQ. ' ') THEN  !ZAMBRESKI 2021
            IKCB = 'N'
         ENDIF
...
!     from first run
      IF (INDEX('FQ',RNMODE) <= 0 .OR. CONTROL % RUN == 1) THEN
		...
		ISWITCH % IKCB   = IKCB    !forced KCB data file (ZAMBRESKI)
...
!  60       FORMAT (I3,11X,9(5X,A1))
			...
			READ (CHARTEST,'(73X,A1)',IOSTAT=ERRNUM) IKCB
            CALL CHECK_A('IKCB  ', IKCB, ERRNUM, MSG, NMSG)
...
!     Fill ISWITCH variable (complete)
	  ...
	  IF (IKCB   /= ' ' .AND. IKCB   /= '.') ISWITCH % IKCB   = IKCB
...
!=======================================================================
	  ...
	  CASE('IKCB');   MSG_TEXT="Option to read KCB from file  "
