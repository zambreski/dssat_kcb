...
!     Calls appropriate potential evapotranspiration routine
     ...
     &      EO)      !Output
      ...
      REAL CANHT, CLOUDS, EO, EORATIO, ET_ALB, SRAD, TAVG, TDEW, ETO,REFET
      REAL TMAX, TMIN, VAPR, WINDHT, WINDSP, XHLAI, KCB
      REAL WINDRUN, XLAT, XELEV, KC
      ...
      XLAT   = WEATHER % XLAT
      XELEV  = WEATHER % XELEV
	  ETO    = WEATHER % ETO  	    ! ZAMBRESKI 2021 
...
!             with KC = 1.0
     ...
     &        TMAX, TMIN, VAPR, WINDSP, WINDHT, XHLAI,IKCB,!Input
     &        EO)                                         !Output
!         ------------------------
...
!         ------------------------
     ...
     &        EO)                                         !Output
!         ------------------------
          !Observed Reference ET from weather file  (ZAMBRESKI 2021)
          CASE ('O')
		  
			  REFET = ETO	! Assuming short-crop reference ET		  
			  SELECT CASE (IKCB)
				CASE('Y')            ! Forced values of KCB
			  ...
			  ! Convert to potential ET rate using crop coefficient
			  EO = REFET*KC
			  EO = MAX(EO,0.0001)
...
!-----------------------------------------------------------------------
!     LOCAL VARIABLES:
      REAL TAVG, PATM, PSYCON, UDELTA, EMAX, EMIN, ES, EA, FC, FEW, FW,ETO
